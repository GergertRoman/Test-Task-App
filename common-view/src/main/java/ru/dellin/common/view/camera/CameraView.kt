package ru.dellin.common.view.camera
...
class CameraView @JvmOverloads constructor(
  ...
  private var currentCamera: Camera = Camera.CAMERA_BACK
        ...
        context.getExternalFilesDir(PATH_SAVE_IMAGES + imageDir.orEmpty())
          ?.also {
            imageReader?.setOnImageAvailableListener(
              JpegImageAvailable(it) { pathImage -> callbackSaveImage?.invoke(pathImage) },
              null
            )
          }
  ...
  private fun chooseOptimalSize(
    ...
    return if (suitableSizes.size > 0) {
      if (hasTopLimit) {
        Collections.max(suitableSizes, CompareSizesByArea())
      } else {
        Collections.min(suitableSizes, CompareSizesByArea())
      }
    } else {
      choices[0]
    }
